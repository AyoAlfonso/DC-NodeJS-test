...
*/
let rsmq = new RedisSMQ({client: redis})
...
*/
exports.sendMessage = async ({queueName, orderDetailId}) => {
 try
    { await rsmq.sendMessage({qname: queueName+'Notifications', message: orderDetailId}) 
      await rsmq.sendMessage({qname: queueName+'Orders', message: orderDetailId})
...
exports.deleteOrder = async ({queueName, messageid}) => {
try {
   await rsmq.deleteMessage({qname:"myqueue", id:messageid})
        if (resp==1) {
